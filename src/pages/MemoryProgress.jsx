...
const MemoryProgress = () => {
  ...
  return (
    <div className="memory-progress-page">
      ...
      {summaryOpen && (
        <div className="activity-summary-overlay" role="presentation" onClick={() => setSummaryOpen(false)}>
          <section
          ...
          >
            <div className="activity-summary-header">
              <div>
                <p>CARE CIRCLE SUMMARY</p>
                <h2 id="activity-summary-title">This week at a glance</h2>
              </div>
              <button type="button" className="activity-summary-close" onClick={() => setSummaryOpen(false)} aria-label="Close summary">
                <X size={19} />
              </button>
            </div>

            <p className="activity-summary-lead">
              {snap.todayPlays
                ? `There ${snap.todayPlays === 1 ? 'was' : 'were'} ${snap.todayPlays} memory activity ${snap.todayPlays === 1 ? 'logged' : 'activities logged'} today, alongside ${snap.routineDone} completed routine item${snap.routineDone === 1 ? '' : 's'}.`
                : `No memory activity has been logged today. ${snap.routineDone} routine item${snap.routineDone === 1 ? ' is' : 's are'} marked complete.`}
            </p>

            <div className="activity-summary-stats">
              <div><span>Overall progress</span><strong>{snap.overall}%</strong></div>
              <div><span>Activities this week</span><strong>{snap.activities} / {snap.planned}</strong></div>
              <div><span>Accuracy</span><strong>{snap.accuracy}%</strong></div>
              <div><span>Current streak</span><strong>{snap.streak} days</strong></div>
            </div>

            <div className="activity-summary-note">
              <Lightbulb size={18} />
              <p>This summary supports conversations with family or a health worker. It is not a medical diagnosis.</p>
            </div>

            <button type="button" className="activity-summary-done" onClick={() => setSummaryOpen(false)}>Done</button>
          </section>
        </div>
      )}
