...
import { loadCareAssessment, saveCareAssessment } from '../../lib/careAssessmentsApi';
...
export default function CaregiverAssessment() {
  ...
  useEffect(() => {
    ...
    loadCareAssessment()
      .then((live) => {
        ...
        const fallback = getAssessmentForPatient('aita');
        const next = {
          ...fallback,
          patientId: live.patientId,
          patientName: live.patient.display_name || 'Patient',
          age: live.age ?? fallback.age,
          functional: { ...fallback.functional, ...live.functional },
          motor: { ...fallback.motor, ...live.motor },
          memoryQuiz: live.memoryQuiz || fallback.memoryQuiz,
          memoryJourney: live.memoryJourney || fallback.memoryJourney,
          lastUpdated: live.savedAt || fallback.lastUpdated,
        };
        setLiveConnection(live);
        setSelectedPatientId(live.patientId);
        setAssessment(next);
        setReport(live.latestEvaluation || null);
      })
      .catch(() => {});
    return () => { alive = false; };
  }, []);

  const persistLiveAssessment = (next, latestEvaluation = null) => {
    if (!liveConnection?.patientId) return;
    saveCareAssessment({
      patientId: liveConnection.patientId,
      assessment: next,
      latestEvaluation,
      notes: liveConnection.notes || '',
    }).catch(() => {});
  };
  ...
  const handleRunEvaluation = async () => {
    ...
    try {
      ...
      setReport(res);
      persistLiveAssessment(assessment, res);
    } finally {
      setEvaluating(false);
    }
  };
  ...
  const demoMeta = DEMO_PATIENTS[selectedPatientId] || DEMO_PATIENTS.aita;
  const patientMeta = liveConnection
    ? {
      ...demoMeta,
      name: liveConnection.patient.display_name || 'Patient',
      age: assessment.age ?? demoMeta.age,
      village: 'Connected securely',
      caregiver: liveConnection.relationshipRole,
      education_years: assessment.education_years ?? demoMeta.education_years,
      education_label: assessment.education_label || demoMeta.education_label,
    }
    : demoMeta;
    ...
    <div className="os-page ss-assessment-page" style={{ padding: '0 24px 32px' }}>
      ...
      <div className="os-chart-strip" style={{ marginTop: 16 }}>
        ...
        <div className="os-chart-id">
          <p className="os-kicker">{liveConnection ? 'Live care record · 10-Item FAQ (ADL)' : 'Watching with care · 10-Item FAQ (ADL)'}</p>
          ...
          {!liveConnection && <div className="os-chart-actions">
