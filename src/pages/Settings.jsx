...
import {
  ...
  Download, Trash2, ShieldAlert, Camera, Users, Send, Check,
} from 'lucide-react';
...
import { acceptCareInvite, createCareInvite, listReceivedCareInvites, listSentCareInvites } from '../lib/careRelationshipsApi';
...
function Settings() {
  ...
  const [careRole, setCareRole] = useState('caregiver');
  const [sentCareInvites, setSentCareInvites] = useState([]);
  const [receivedCareInvites, setReceivedCareInvites] = useState([]);
  ...
  const refreshCareInvites = async () => {
    if (!session?.id) {
      setSentCareInvites([]);
      setReceivedCareInvites([]);
      return;
    }
    try {
      const [sent, received] = await Promise.all([listSentCareInvites(), listReceivedCareInvites()]);
      setSentCareInvites(sent);
      setReceivedCareInvites(received);
    } catch {
      // The account can still use local preferences if the network is unavailable.
    }
  };
  ...
  const sendCareInvite = async (event) => {
    event.preventDefault();
    setCareBusy(true);
    setCareNote('');
    try {
      await createCareInvite({ email: careEmail, relationshipRole: careRole });
      setCareEmail('');
      setCareNote('Invitation saved. Ask this person to sign in with the invited email, then accept it in Care Sharing.');
      await refreshCareInvites();
    } catch (err) {
      setCareNote(err.message || 'Could not create the invitation.');
    } finally {
      setCareBusy(false);
    }
  };
  ...
  const acceptInvite = async (inviteId) => {
    ...
    try {
      ...
      setCareNote('Care relationship connected securely.');
      await refreshCareInvites();
    } catch (err) {
      setCareNote(err.message || 'Could not accept the invitation.');
    } finally {
      setCareBusy(false);
    }
  };
    ...
    <div className="granth-page settings-page">
      ...
      <section className="settings-section">
        <div className="settings-section-head">
          ...
          <div>
            <h3>Account</h3>
            <p>Optional. Sign in with your email link to securely save and share care data. You can keep using the app as a guest.</p>
          </div>
        </div>
      ...
      {session?.verified && (
        <section className="settings-section">
          <div className="settings-section-head">
            ...
            <div>
              <h3>Care sharing</h3>
              <p>Invite a trusted caregiver or doctor. They can connect only after accepting with this email address.</p>
            </div>
          </div>
          <div className="granth-page-card settings-card">
            <form className="settings-care-form" onSubmit={sendCareInvite}>
              <label className="settings-field">
                <span>Care team email</span>
                <input type="email" value={careEmail} onChange={(event) => setCareEmail(event.target.value)} placeholder="caregiver@example.com" required />
              </label>
              <label className="settings-field">
                <span>Role</span>
                <select value={careRole} onChange={(event) => setCareRole(event.target.value)}>
                  <option value="caregiver">Caregiver</option>
                  <option value="doctor">Doctor</option>
                </select>
              </label>
              <button type="submit" className="btn btn-primary" disabled={careBusy}>
                <Send size={14} /> {careBusy ? 'Saving…' : 'Create secure invitation'}
              </button>
            </form>

            {!!sentCareInvites.length && (
              <div className="settings-care-list">
                <h4>Pending invitations</h4>
                {sentCareInvites.filter((invite) => invite.status === 'pending').map((invite) => (
                  <p key={invite.id}>{invite.email} · {invite.relationship_role} · awaiting acceptance</p>
                ))}
              </div>
            )}

            {!!receivedCareInvites.length && (
              <div className="settings-care-list">
                <h4>Invitations for you</h4>
                {receivedCareInvites.map((invite) => (
                  <div className="settings-care-received" key={invite.id}>
                    <span>Invitation to support a patient as their {invite.relationship_role}</span>
                    <button type="button" className="btn btn-secondary" disabled={careBusy} onClick={() => acceptInvite(invite.id)}>
            ...
            {careNote && <p className="settings-data-note">{careNote}</p>}
          </div>
        </section>
      )}
