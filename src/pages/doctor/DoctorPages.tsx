...
function NotesTab({ patient }) {
  const [extraNotes, setExtraNotes] = useState<unknown[]>([]);
  ...
  const handleSend = () => {
    ...
    bffSilent('POST', '/api/v1/doctor/notes', {
  ...
  return (
    <ChartShell
      patient={patient}
      rail={<Panel title="Author"><p className="os-note-body">Session notes stay with Dr. Meera Sharma. Sending a note alerts the care circle.</p></Panel>}
    >
      <Panel title={`Send a note to ${first}’s care circle`}>
        <label className="os-note-body" htmlFor={`dr-note-${patient.id}`}>Clinical advice for the family</label>
        <textarea
          id={`dr-note-${patient.id}`}
          className="os-note-draft"
          placeholder={`Calm, clear instructions for ${patient.caregiver || 'the caregiver'}…`}
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          rows={3}
        />
        <div className="os-note-send-row">
          <p className="os-meta">{sent ? 'Shared with the care circle. Caregiver notified.' : 'Caregiver gets a high-priority alert when you send.'}</p>
          <button type="button" className="os-full-btn" style={{ width: 'auto' }} onClick={handleSend} disabled={!draft.trim()}>
            Send note to caregiver
          </button>
        </div>
      </Panel>
      <Panel title={`Notes about ${first}`}>
        {rows.length === 0 ? (
          <p className="os-empty">No session notes yet — write the first after you see {first}.</p>
        ) : (
