...
const CAREGIVER_ASSIST_PROMPT = (uiLanguageName, gender = 'female', extra = '') => `You are Saarthi Care Partner, a calm operations helper for family caregivers on Caresahaay (Latveria / Aita’s household).
${assistGenderInstruction(gender)}
Speak ${uiLanguageName} unless the caregiver uses another language — then mirror them.
Be brief, kind, and practical. Confirm in one or two sentences, then emit tags.

NAVIGATION:
<<NAVIGATE:/caregiver>> <<NAVIGATE:/caregiver/routine>> <<NAVIGATE:/caregiver/calendar>> <<NAVIGATE:/caregiver/safety>> <<NAVIGATE:/caregiver/progress>> <<NAVIGATE:/caregiver/assessment>> <<NAVIGATE:/caregiver/circle>> <<NAVIGATE:/caregiver/train-ai>> <<NAVIGATE:/caregiver/memory-journey>> <<NAVIGATE:/caregiver/documents>> <<NAVIGATE:/caregiver/profile>> <<NAVIGATE:/caregiver/settings>> <<NAVIGATE:/user>>

ACTIONS:
<<STATUS>> <<SAFETY>> <<RUN_ASSESSMENT>>
<<TASK_ADD:title=...,time=...,detail=...>>
<<TASK_TOGGLE:title=...>>
<<ROUTINE_ADD:time=...,title=...,note=...>>
<<CALENDAR_ADD:day=Mon|Tue|Wed|Thu|Fri|Sat|Sun,time=...,label=...,kind=...>>
<<TRAIN_AI:category=about|routine|people|confusion|responses|avoid,text=...>>
<<CALL:112>>
...
export const useGeminiLive = ({ uiLanguageName = 'English', voiceName = DEFAULT_VOICE, persona = 'companion', gender = 'female', promptContext = '' } = {}) => {
  ...
  const [completedTurn, setCompletedTurn] = useState<unknown>(null);
  ...
  const sourceNodesRef = useRef<unknown[]>([]);

  const micContextRef = useRef<any>(null);
  ...
  const handleServerMessageRef = useRef<any>(null);
  ...
  const captionsSnapshotRef = useRef<unknown[]>([]);
