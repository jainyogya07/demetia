import { hydrateSupabaseRoutine, recordSupabaseRoutineEvent } from './routinesApi';
...
/** Pull today's saved routine state after a Supabase user signs in. */
export async function hydrateRoutineBackend(now = new Date()) {
  try {
    const completed = await hydrateSupabaseRoutine(ROUTINE_TEMPLATE, now);
    if (!completed) return false;
    const all = readJson(ROUTINE_KEY, {});
    writeJson(ROUTINE_KEY, { ...all, [todayKey(now)]: completed });
    pingLive();
    return true;
  } catch {
    return false;
...
export function markRoutineDone(id, now = new Date()) {
  ...
  recordSupabaseRoutineEvent(id, 'done').catch(() => {});
...
export function unmarkRoutineDone(id, now = new Date()) {
  ...
  delete row[id];
  writeJson(ROUTINE_KEY, { ...all, [day]: row });
  recordSupabaseRoutineEvent(id, 'dismissed', { reason: 'manual-undo' }).catch(() => {});
  return getRoutineItems(now);
}
