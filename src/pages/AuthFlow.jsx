...
export default function AuthFlow({ variant = 'page', onSkip, requireAccount = false }) {
  ...
  const sendSupabaseLink = async ({ createUser, profile = {} }) => {
    if (!supabase) throw new Error('Supabase is not configured.');
    const destination = `${window.location.origin}/user`;
    const { error: linkError } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        data: createUser ? {
          ...
          phone: digitsPhone(profile.phone),
    ...
    setOtpBack(createUser ? 'signup' : 'login');
    setPane('email-link');
    setHint('We sent a secure sign-in link. Open it in this browser to finish.');
  };
  ...
  const sendSignup = async (event) => {
    ...
    try {
      if (isSupabaseConfigured) {
        await sendSupabaseLink({
          createUser: true,
          profile: { firstName, lastName, birthDate, phone },
        });
        return;
      }
      let data;
  ...
  const sendLogin = async (event) => {
    ...
    try {
      if (isSupabaseConfigured) {
        await sendSupabaseLink({ createUser: false });
        return;
      }
      let data;
      try {
      ...
      <Reveal className="af-card">
        ...
        {pane !== 'otp' && (
          <>
            ...
            <p className="af-lead">
              {pane === 'login'
                ? (isSupabaseConfigured ? 'Enter your email. We will send a secure sign-in link.' : 'Name and mobile. A 6-digit code goes to your email — if mail is unavailable, the code shows on the next screen.')
                : (isSupabaseConfigured ? 'Your details are stored securely. We will send a secure sign-in link to your email.' : 'Name, birthday, phone (saved on the account), email for the OTP. If email cannot send, the code appears on the next screen.')}
            </p>
          </>
        )}
        ...
        {pane === 'login' && (
          <form className="af-form" onSubmit={sendLogin}>
            ...
            <label>
              Email
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required={isSupabaseConfigured} placeholder={isSupabaseConfigured ? 'Your account email' : 'Where we mailed your last code'} autoComplete="email" />
            </label>
            <button type="submit" className="af-primary" disabled={busy}>{busy ? 'Please wait…' : isSupabaseConfigured ? 'Email me a sign-in link' : 'Log in'}</button>
        ...
        {pane === 'email-link' && (
          <div className="af-form">
            <p className="af-kicker">Check your email</p>
            <h1>Open your secure link</h1>
            <p className="af-lead">
              We sent a sign-in link to <strong>{magicEmail}</strong>. Open it in this browser; you will return to Smriti Saarthi automatically.
            </p>
            <button type="button" className="af-primary" disabled={busy} onClick={() => setPane(otpBack === 'login' ? 'login' : 'signup')}>
              Back
            </button>
          </div>
        )}
