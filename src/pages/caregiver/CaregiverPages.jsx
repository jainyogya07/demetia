...
import { loadCaregiverDashboard } from '../../lib/caregiverDashboardApi';
import {
...
function LiveCareOverview({ data }) {
  const patientName = data.patient?.display_name || 'Patient';
  const next = data.routine.find((item) => !item.completed);
  return (
    <div className="os-page">
      <SyncBar asOf="Live Supabase data" lastSync="just now" extra={`${data.relationshipRole} access`} />
      <section className="os-chart-strip">
        <AvatarSlot name={patientName} size={56} label={patientName} />
        <div className="os-chart-id">
          <p className="os-kicker">Connected care record</p>
          <h2>{patientName}'s day</h2>
          <p>Routine and reminder status shared securely with you.</p>
        </div>
        <div className="os-chart-actions"><Badge tone="stable">Connected</Badge></div>
      </section>
      <div className="os-kpis">
        <Stat label="Completed today" value={`${data.completed} / ${data.routine.length}`} hint="Saved routine events" />
        <Stat label="Due now" value={data.due} hint={data.due ? 'Needs a gentle check-in' : 'Nothing overdue'} />
        <Stat label="Next" value={next?.time || 'All done'} hint={next?.title || 'A quiet rest of day'} />
        <Stat label="Access" value={data.relationshipRole} hint="Accepted care connection" />
      </div>
      <Panel title={`${patientName}'s live routine`}>
        <DataTable
          empty="No active routine items yet."
          columns={[
            { key: 'time', label: 'Time', narrow: true },
            { key: 'category', label: 'Type', narrow: true, render: (row) => <Badge>{row.category}</Badge> },
            { key: 'title', label: 'Routine', render: (row) => <span><strong>{row.title}</strong><em className="os-sub">{row.description}</em></span> },
            { key: 'status', label: 'Status', render: (row) => <Badge tone={row.completed ? 'stable' : row.status === 'Due' ? 'urgent' : 'watch'}>{row.status}</Badge> },
          ]}
          rows={data.routine}
        />
      </Panel>
...
export function CgOverview() {
  const liveCare = useLiveCareDashboard();
  useStoreTick();
  ...
  if (liveCare.data) return <LiveCareOverview data={liveCare.data} />;
  return (
    <div className="os-page">
      <SyncBar asOf={liveCare.loading ? 'Connecting to care records…' : `${CG_LIVE.asOf} · ${CG_LIVE.clock}`} lastSync={CG_LIVE.lastSync} extra={CG_LIVE.ping} />
      <PersonStrip lead="Household desk · mother in care" />
      <div className="os-kpis">
        <Stat label="Latveria" value="Home" hint={CG_PATIENT.lastCheckIn} />
        <Stat label="Still open" value={open} hint="Today’s checklist" />
