...
export default function SpotDifference({ onBack }) {
  ...
  const [startTime, setStartTime] = useState<unknown>(null);
  const [elapsed, setElapsed] = useState(0);
  const [finished, setFinished] = useState(false);
  const [markers, setMarkers] = useState<unknown[]>([]);
  const [wrongMarker, setWrongMarker] = useState<unknown>(null);

  const canvasARef = useRef<any>(null);
  const canvasBRef = useRef<any>(null);
