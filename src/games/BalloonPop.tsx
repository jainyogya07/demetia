...
export default function BalloonPop({ onBack }) {
  ...
  const [balloons, setBalloons] = useState<unknown[]>([]);
  const [floatingScores, setFloatingScores] = useState<unknown[]>([]);
  const [lastItem, setLastItem] = useState<unknown>(null);

  const skyRef = useRef<any>(null);
  const animRef = useRef<any>(null);
  const balloonsRef = useRef<unknown[]>([]);
