...
export default function MemoryJourney({
  ...
  const [lastTurnFeedback, setLastTurnFeedback] = useState(null); // { correct, direction, expected }
  const [turnTelemetry, setTurnTelemetry] = useState([]);
  const [landmarkTelemetry, setLandmarkTelemetry] = useState([]);
  const [recallIndex, setRecallIndex] = useState(0);
  const [selectedLandmarkAnswer, setSelectedLandmarkAnswer] = useState(null);
