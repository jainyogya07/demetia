...
function MemoryQuiz({ onComplete, onSkip }) {
  ...
  const [language, setLanguage] = useState(() => (
    LANGUAGE_NAMES[appLang] ? appLang : "hi"
  ));
  ...
  useEffect(() => {
    ...
    try {
      ...
      import('../lib/bff').then(({ postGameSession }) => {
        postGameSession({
          gameId: 'memory-quiz',
          score: result.percentage,
          extra: { score: result.score, total: result.totalQuestions },
        });
      }).catch(() => {});
    } catch {
      try {
  ...
  return (
    <div className="memory-quiz-overlay">
      <div className="memory-quiz-card">
        ...
        {onSkip && (
          <button type="button" className="memory-skip-today" onClick={onSkip}>
            Later today
          </button>
        )}
