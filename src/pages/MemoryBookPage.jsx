...
import { createSupabaseMemory, loadSupabaseMemories } from '../lib/memoriesApi';
import './MemoryBookPage.css';

async function fetchMemories() {
  const supabaseRows = await loadSupabaseMemories();
  if (supabaseRows) return { rows: supabaseRows, source: 'supabase' };
...
export default function MemoryBookPage() {
  ...
  const saveMemory = async (fields) => {
    ...
    try {
      const supabaseRow = await createSupabaseMemory(payload, photoFile);
      if (supabaseRow) {
    ...
    <div className="memory-book-page">
      <header className="memory-book-page-head">
        ...
        <p>
          ...
          {source === 'local' ? ' Local copy.' : source === 'fallback' ? ' API fallback.' : source === 'supabase' ? ' Securely saved in your Supabase account.' : ' Loaded from Postgres.'}
        </p>
      </header>
          ...
          <div className="mb-photo-picker-main">
            ...
            {!usingDefaultPhoto && (
              <button
                ...
                onClick={() => {
                  setPhotoFile(null);
                  setForm((prev) => ({ ...prev, photo_url: DEFAULT_MEMORY_PHOTO }));
                }}
              >
                Remove photo
              </button>
