...
export default function DailyRoutineDashboard() {
  ...
  const [justCompletedId, setJustCompletedId] = useState<unknown>(null);

  useEffect(() => subscribeLive(() => setTick((n) => n + 1)), []);
  useEffect(() => { bffFetch('/api/v1/routine/today'); }, []);
  ...
  const handleToggle = (item) => {
    ...
    pingLive();
    // Non-blocking sync to polyglot Node.js backend
    bffSilent('PATCH', `/api/v1/routine/tasks/${encodeURIComponent(item.id)}/toggle`);
  };
