...
import { getDashboardSummary } from '../lib/bff';
...
function UserDashboard() {
  ...
  const greeting = summary?.greeting || t(greetingChromeKey());

  useEffect(() => {
    let alive = true;
    getDashboardSummary().then((row) => {
      if (alive) setSummary(row);
    });
    return () => { alive = false; };
  }, []);
              ...
              <Reveal delay={0.01}>
                <div className="ss-dash-greeting">
                  <BlurText
                    ...
                    delay={40}
                  />
                  <p className="ss-dash-greet-sub">
                    {summary?.safety?.copy || t('chrome.calmDaySub')}
                    {summary?.routineSnippet?.title ? ` · ${summary.routineSnippet.title}` : ''}
                  </p>
                </div>
              </Reveal>
            </div>
