...
import { Mic, MessageSquare, Compass, Sun, Heart, Sparkles, CheckCircle2, CalendarDays } from 'lucide-react';
import { useAppNav } from '../AppNavContext';
import { useI18n } from '../I18nContext';
import companionPortrait from '../assets/infinity_pfp.jpg';
import { getRoutineItems } from '../lib/liveState';
...
export default function VoiceAgentCard() {
  ...
  return (
    <section className="voice-companion-card ss-hero-agent-card">
      ...
      <div className="voice-today-summary">
        <div className="voice-today-copy">
          <div className="voice-today-icon">
            <CheckCircle2 size={18} />
          </div>
          <div>
            <span>{t('chrome.myDay')}</span>
            <strong>{t('routineCard.stepsTaken', { done: completedCount, total: routine.length })}</strong>
          </div>
        </div>
        <button type="button" onClick={() => openModule('routine')}>
